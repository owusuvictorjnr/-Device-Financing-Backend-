...
export class CustomersService {
  ...
  async create(
    ...
    const agentId = await this.resolveTargetAgentId(
      actor,
  ...
  async update(
    ...
    if (updateCustomerDto.agentId !== undefined) {
      targetAgentId = await this.resolveTargetAgentId(
        actor,
    ...
    try {
      const updatedCustomer = await this.prisma.customer.update({
        where: { id },
        data: {
          ...(updateCustomerDto.userId !== undefined
            ? { user_id: updateCustomerDto.userId }
            : {}),
          ...(targetAgentId ? { agent_id: targetAgentId } : {}),
          ...(updateCustomerDto.nationalId !== undefined
  ...
  private mapCustomerToResponseDto(
    customer: CustomerRecord,
  ): CustomerResponseDto {
    return {
      id: customer.id,
